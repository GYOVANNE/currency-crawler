<?php
namespace App\Domains\Currency\Adapters;

use App\Domains\Currency\Ports\StrategyInterface;
use Symfony\Component\DomCrawler\Crawler;

class SingleCodeFilterStrategy implements StrategyInterface {
    ...
    public function execute($html, $codes):array {
        ...
        $crawler->filter('table')->filter('tbody')->filter('tr')->each(function ($tr) use ($codes, &$currencyData) {
            ...
            $currencyCodeNode = $tr->filter('td:nth-child(1)');
            ...
            if ($currencyCode != $codes) {
                return;
            }

            $number = $tr->filter('td:nth-child(2)')->text();
            ...
            $currencyData[] = [
                'code' => trim($currencyCodeNode->text()),
